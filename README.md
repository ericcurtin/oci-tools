...
`--build-arg` works; multiple explicit sources and glob patterns in one
`COPY`/`ADD` both work too (each landing under the destination by its
own basename; glob matching verified byte-for-byte against Go's own
`path/filepath.Match`, the exact matcher real BuildKit itself uses);
`COPY --from=<external-image>` is not implemented yet). Milestone 3
also grew real
`--memory-swap`/
...
| 4 | `oci-dockerfile`; `ociman build` (multi-stage, cache) | in progress — `RUN`/`COPY`/`ADD`/`--build-arg` work end to end and commit real layers, including multiple explicit sources and glob patterns in one `COPY`/`ADD` (`ADD`'s remote-URL sources and the build cache are not yet implemented) (see `docs/design/0050`-`0060`, `0068`, `0072`-`0073`) |
