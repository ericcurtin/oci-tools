...
`RUN` executes for real; `COPY` from the build context works for a
single file/directory source; multi-stage builds work when a later
stage's own `FROM` references an earlier one by name; `ADD`,
`COPY --from=`, and multi-source/glob `COPY` are not implemented yet).
See [docs/design/](docs/design/) for design notes per milestone.
...
| 4 | `oci-dockerfile`; `ociman build` (multi-stage, cache) | in progress — `RUN`/`COPY` work end to end and commit real layers; multi-stage builds work via `FROM <earlier-stage>` (not yet `COPY --from=`); `ADD` and the build cache are not yet implemented (see `docs/design/0050`-`0053`) |
