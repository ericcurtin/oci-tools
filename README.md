...
| 2 | `oci-spec-types`/`oci-registry`/`oci-store`; `ociman pull/images/inspect` | **done** (plus `ociman rmi`, including refusing to remove an image a container still depends on unless `--force`, beyond the original scope — see `docs/design/0102`) |
...
| 4 | `oci-dockerfile`; `ociman build` (multi-stage, cache) | in progress — `RUN`/`COPY`/`ADD`/`--build-arg`/`--target` work end to end and commit real layers, including multiple explicit sources, glob patterns, `COPY --from=<external-image>`, `ADD` from a remote URL, multiple `ARG` names on one line, `COPY`/`ADD --chmod`/`--chown`, and a real local build cache (`--no-cache` to disable) (see `docs/design/0050`-`0060`, `0068`, `0072`-`0079`, `0097`, `0101`) |
