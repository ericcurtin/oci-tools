...
Milestone 5 also now has its first two real pieces: `oci-erofs` builds
verified-deterministic erofs images via `mkfs.erofs` and seals/verifies
them with real fs-verity ioctls. See [docs/design/](docs/design/) for
...
| 5 | erofs/mount/BLS; `ociboot install to-disk`; dracut module; QEMU boot test | in progress — `oci-erofs` builds real, verified-deterministic erofs images via `mkfs.erofs`, and seals/verifies them with real fs-verity ioctls (see `docs/design/0061`-`0062`); the dm-verity fallback, `ociboot`'s own subcommands, and the dracut module are not started yet |
