...
# TEMPORARY diagnostic: passt is still dying at startup even with
# --runas 0:0; wrap it in strace so the next failed run's log shows
# exactly which syscall/event is responsible now, instead of guessing
# further. ci/run-in-vm.sh points OCIVMM_PASST at this wrapper and
# cats the trace on failure.
