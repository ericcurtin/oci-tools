...
# TEMPORARY diagnostic: passt has been dying deterministically and
# immediately in this exact CI environment; wrap it in strace so the
# next failed run's log shows exactly which syscall/event is
# responsible, instead of guessing further. ci/run-in-vm.sh points
# OCIVMM_PASST at this wrapper and cats the trace on failure.
