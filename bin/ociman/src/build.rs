...
fn build_stage(
    ...
    let build_dir = if needs_rootfs {
        ...
        // DNS resolution to reach a real package registry, the same
        // real functional need `ociman run`'s own identical default
        // serves (see its own doc comment for why this is meaningful,
        // not cosmetic, given this project's own containers already
        // share the host's real network namespace unmodified).
        // `--dns none` (exactly one value, case-insensitive, and
        // nothing else given alongside it) skips this entirely,
        // matching real buildah's own checked-directly rule exactly
        // (`~/git/podman/vendor/go.podman.io/buildah/run_linux.go`:
        // `len(b.CommonBuildOpts.DNSServers) != 1 ||
