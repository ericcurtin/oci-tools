...
pub struct VmSpec {
    ...
    /// An already-connected unix-stream fd to passt (inherited from
    /// the parent `ocivmm run` process), backing the guest's one
    /// virtio-net device. Connected parent-side so a passt failure is
    /// a clear pre-boot error instead of a mid-boot device activation
    /// failure — and so nothing depends on the socket *file* still
    /// existing by the time the guest driver comes up.
    pub passt_fd: i32,
}
...
mod imp {
    ...
    pub fn boot(spec: &VmSpec) -> anyhow::Result<std::convert::Infallible> {
        ...
        // configures it (fd flavor). No vsock/TSI: that needs
        // libkrunfw's patched kernel, and this VMM only ever boots
        // distro kernels.
        vmr.add_network_interface(NetworkInterfaceConfig {
            iface_id: "eth0".to_string(),
            backend: VirtioNetBackend::UnixstreamFd(spec.passt_fd),
            mac: GUEST_MAC,
