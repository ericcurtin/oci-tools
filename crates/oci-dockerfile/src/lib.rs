...
//!   (`RUN --mount=`, `COPY --link`/`--parents`, `ADD --link`/
//!   `--keep-git-dir`/`--unpack`) — a Containerfile using any of
//!   these fails to parse with a clear error, rather than being
//!   silently misparsed. `ADD --checksum=` and `COPY`/`ADD
//!   --exclude=` are now parsed here and enforced/applied by
//!   `bin/ociman/src/build.rs`.
