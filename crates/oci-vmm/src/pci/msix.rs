...
// Ported into oci-vmm from Firecracker (src/vmm/src/pci/msix.rs), trimmed of
// metrics/snapshot/ACPI. byteorder/zerocopy uses were replaced with core
// `to_le_bytes`/`from_le_bytes`.

//! MSI-X emulation: the MSI-X capability structure, vector table and pending
//! bit array, wired to KVM irqfd routing through
//! [`crate::vstate::interrupts::MsixVectorGroup`].

// `unsafe` is required for the `ByteValued` impl of `MsixCap`, which is a
// plain-old-data struct where any byte pattern is valid.
#![allow(unsafe_code)]
...
impl MsixConfig {
    ...
    pub fn write_msg_ctl_register(&mut self, offset: u8, data: &[u8]) {
        if offset == 2 && data.len() == 2 {
            ...
            self.set_msg_ctl(u16::from_le_bytes(data.try_into().unwrap()));
            ...
            self.set_msg_ctl((u32::from_le_bytes(data.try_into().unwrap()) >> 16) as u16);
        }
    }
