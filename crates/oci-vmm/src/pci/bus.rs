...
// Ported into oci-vmm from Firecracker (src/vmm/src/pci/bus.rs), trimmed of
// metrics/snapshot/ACPI. Firecracker implements `crate::vstate::bus::BusDevice`
// for `PciConfigIo`/`PciConfigMmio`; here the same `read`/`write` signatures
// are provided as inherent methods so the builder can hook them into whatever
// port-IO/MMIO dispatch it uses.

//! PCI bus emulation: the root bridge, device slot bookkeeping, and the
//! legacy conf1 (0xcf8/0xcfc) and MMIO configuration access mechanisms.
...
pub enum PciRootError {
    ...
    /// PCI device ID is already in use.
    #[error("PCI device ID {0} is already in use.")]
...
impl PciConfigIo {
    ...
    pub fn config_space_write(&mut self, offset: u64, data: &[u8]) -> Option<Arc<Barrier>> {
        if offset.saturating_add(data.len() as u64) > 4 {
            return None;
        }
    ...
    /// Handle a Port IO read of the 0xcf8..0xd00 range. `offset` is relative to 0xcf8:
    /// offsets 0..=3 read CONFIG_ADDRESS, offsets 4..=7 read CONFIG_DATA.
    pub fn read(&mut self, _base: u64, offset: u64, data: &mut [u8]) {
    ...
    /// Handle a Port IO write of the 0xcf8..0xd00 range. `offset` is relative to 0xcf8:
    /// offsets 0..=3 write CONFIG_ADDRESS, offsets 4..=7 write CONFIG_DATA.
    pub fn write(&mut self, _base: u64, offset: u64, data: &[u8]) -> Option<Arc<Barrier>> {
        // `offset` is relative to 0xcf8
        match offset {
            o @ 0..=3 => {
...
impl PciConfigMmio {
    ...
    /// Handle an MMIO read inside the ECAM window. `offset` is relative to the window base.
    pub fn read(&mut self, _base: u64, offset: u64, data: &mut [u8]) {
        // Only allow reads to the register boundary.
        let start = (offset as usize) % 4;
        let end = start + data.len();
    ...
    /// Handle an MMIO write inside the ECAM window. `offset` is relative to the window base.
    pub fn write(&mut self, _base: u64, offset: u64, data: &[u8]) -> Option<Arc<Barrier>> {
