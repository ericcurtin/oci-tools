...
// Ported into oci-vmm from Firecracker (src/vmm/src/pci/configuration.rs),
// trimmed of metrics/snapshot/ACPI. byteorder/zerocopy uses were replaced with
// core `to_le_bytes`/`from_le_bytes`.

//! PCI configuration space emulation: register file, capability list plumbing
//! and BAR programming.
...
impl Bars {
    ...
    pub fn write(&mut self, bar_idx: u8, offset: u8, data: &[u8]) {
        ...
        if data.len() == 4 && u32::from_le_bytes(data.try_into().unwrap()) == 0xffff_ffff {
    ...
    pub fn read(&mut self, bar_idx: u8, offset: u8, data: &mut [u8]) {
        ...
        let bytes = if bar.about_to_be_read {
            ...
            bar.encoded_size.to_le_bytes()
        } else {
            bar.encoded_addr.to_le_bytes()
        };
...
impl PciConfiguration {
    ...
    pub fn write_config_register(&mut self, reg_idx: u16, offset: u8, data: &[u8]) {
        ...
        match data.len() {
            ...
            4 => self.write_reg(reg_idx, u32::from_le_bytes(data.try_into().unwrap())),
            _ => (),
        }
    }
}
...
mod tests {
    ...
    fn test_bars_add_pci_bar() {
        ...
        let mut buf = [0u8; 4];
        bars.read(0, 0, &mut buf);
        assert_eq!(u32::from_le_bytes(buf) & 0xffff_fff0, 0x0);
        bars.read(1, 0, &mut buf);
