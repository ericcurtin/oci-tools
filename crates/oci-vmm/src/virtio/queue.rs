...
// Ported into oci-vmm from Firecracker (src/vmm/src/devices/virtio/queue.rs), trimmed of metrics/snapshot/MMIO.

//! Virtio queue (split virtqueue) implementation: descriptor chains and
//! avail/used ring accounting via volatile accesses into guest memory.

// The queue accesses the descriptor table and the avail/used rings through
// raw pointers into the guest memory mmap; the pointers are validated (bounds
// and alignment) in `initialize` and every access is volatile and bounded by
// the queue size.
#![allow(unsafe_code)]
...
/// The next descriptor flag: this descriptor chains to another one.
pub const VIRTQ_DESC_F_NEXT: u16 = 0x1;
/// The write-only descriptor flag: the device writes, the driver reads.
pub const VIRTQ_DESC_F_WRITE: u16 = 0x2;

/// Max size of virtio queues offered by this VMM's virtio devices.
...
/// Errors from operating on a virtio queue.
#[derive(Debug, thiserror::Error)]
pub enum QueueError {
    /// Descriptor index out of bounds.
    #[error("Descriptor index out of bounds: {0}.")]
    DescIndexOutOfBounds(u16),
    /// Failed to access guest memory holding the virtio queue.
    #[error("Failed to write value into the virtio queue used ring: {0}")]
    MemoryError(#[from] vm_memory::GuestMemoryError),
    /// A queue pointer is not aligned properly.
    #[error("Pointer is not aligned properly: {0:#x} not {1}-byte aligned.")]
    PointerNotAligned(usize, usize),
    /// Attempt to use a virtio queue that is not marked ready.
    #[error("Attempt to use virtio queue that is not marked ready")]
    NotReady,
    /// The queue has an invalid size.
    #[error("Virtio queue with invalid size: {0}")]
    InvalidSize(u16),
}
...
///
/// Should this error bubble up to the event loop, we exit the VMM, since this could be a
...
pub struct Descriptor {
    ...
    pub flags: u16,
    /// Index of the next descriptor if the next flag is set.
    pub next: u16,
}
...
pub struct Queue {
    ...
    /// Index of the next entry in the avail ring that the device will process.
    pub next_avail: Wrapping<u16>,
    /// Index of the next entry the device will write in the used ring.
...
impl Queue {
    ...
    fn desc_table_size(&self) -> usize {
        size_of::<Descriptor>() * usize::from(self.size)
    }

    fn avail_ring_size(&self) -> usize {
        size_of::<u16>()
            + size_of::<u16>()
            + size_of::<u16>() * usize::from(self.size)
            + size_of::<u16>()
    }
    ...
    fn used_ring_size(&self) -> usize {
        size_of::<u16>()
            + size_of::<u16>()
            + size_of::<UsedElement>() * usize::from(self.size)
            + size_of::<u16>()
    }
    ...
    fn get_aligned_slice_ptr<T, M: GuestMemory>(
        ...
        if addr.0 & (alignment as u64 - 1) != 0 {
            ...
            return Err(QueueError::PointerNotAligned(addr.0 as usize, alignment));
        }
    ...
    unsafe fn used_ring_ring_set(&mut self, index: usize, val: UsedElement) {
        ...
        unsafe {
            self.used_ring_ptr
                .add(size_of::<u16>().unchecked_mul(2))
                .cast::<UsedElement>()
                .add(index)
                .write_volatile(val)
        }
    ...
    pub fn used_ring_avail_event_set(&mut self, val: u16) {
        ...
        unsafe {
            self.used_ring_ptr
                .add(
                    size_of::<u16>().unchecked_mul(2)
                        + size_of::<UsedElement>().unchecked_mul(usize::from(self.size)),
                )
                .cast::<u16>()
                .write_volatile(val)
        }
    ...
    ///
    /// If this function returns an error at runtime, then the guest has requested the VMM
    ...
    /// the error to the user, and hence cannot panic on its own.
    ...
    pub fn write_used_element(
        ...
        if self.size <= desc_index {
            error!("attempted to add out of bounds descriptor to used ring: {desc_index}");
